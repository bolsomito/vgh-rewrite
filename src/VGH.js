...
module.exports = class VGH extends Client {
  constructor (options = {}) {
    super(options)
    ...
    this.listeners = []
    this.strings = []
    this.botOwners = process.env.OWNER_IDS && process.env.OWNER_IDS.split(',')
  ...
  initializeStrings (dirPath) {
    fs.readdirSync(dirPath).forEach(file => {
      if (file.endsWith('.json')) {
        try {
          this.strings = Object.assign(require(`./strings/${file}`))
